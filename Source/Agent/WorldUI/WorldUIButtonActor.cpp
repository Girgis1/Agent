...
AWorldUIButtonActor::AWorldUIButtonActor()
{
...
void AWorldUIButtonActor::OnConstruction(const FTransform& Transform)
{
...
void AWorldUIButtonActor::BeginPlay()
{
	Super::BeginPlay();
	...
	if (HoverCollision)
	{
		HoverCollision->OnComponentBeginOverlap.AddUniqueDynamic(this, &AWorldUIButtonActor::HandleHoverCollisionBegin);
		HoverCollision->OnComponentEndOverlap.AddUniqueDynamic(this, &AWorldUIButtonActor::HandleHoverCollisionEnd);
	}
...
void AWorldUIButtonActor::Tick(float DeltaSeconds)
{
...
void AWorldUIButtonActor::SetHovered(bool bInHovered)
{
...
void AWorldUIButtonActor::SetPressed(bool bInPressed)
{
...
void AWorldUIButtonActor::TriggerClick()
{
	SetPressed(true);
	PressTimeRemaining = FMath::Max(0.01f, ClickPressedDuration);
	OnButtonClicked.Broadcast(Action);
}

void AWorldUIButtonActor::SetRayTraceInteractionEnabled(bool bEnabled)
{
...
void AWorldUIButtonActor::HandleHoverCollisionBegin(
...
void AWorldUIButtonActor::HandleHoverCollisionEnd(
	UPrimitiveComponent* OverlappedComponent,
	AActor* OtherActor,
	UPrimitiveComponent* OtherComp,
	int32 OtherBodyIndex)
...
void AWorldUIButtonActor::RefreshVisualState()
{
	const EWorldUIButtonVisualState NewState = bPressed
		? EWorldUIButtonVisualState::Pressed
		: (bHovered ? EWorldUIButtonVisualState::Hovered : EWorldUIButtonVisualState::Idle);
...
void AWorldUIButtonActor::RefreshInteractionCollision()
{
	if (!HoverCollision)
	{
		return;
...
void AWorldUIButtonActor::UpdateVisualTransform(float DeltaSeconds)
{
	if (!ButtonVisualRoot)
	{
		return;
